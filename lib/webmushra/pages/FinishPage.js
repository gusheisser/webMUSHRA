...
FinishPage.prototype.sendResults = function() {
  ...
  try {
    // ⚡️ 1. Forzar guardado de respuestas del cuestionario
    this.storeParticipantData();

    // ⚙️ 2. Preparar encabezado CSV
    let header = [];
    const participantNames = this.session.participant.name || [];
    header = header.concat(participantNames);  // nombres de campos del cuestionario
    header = header.concat(["trial_id", "stimulus", "score"]);
    let csv = header.join(",") + "\n";

    // 📋 3. Obtener respuestas del participante
    const participantResponses = this.session.participant.response || [];

    // 🎧 4. Recorrer los trials y generar filas
    const trials = this.session.trials || [];
    for (let t of trials) {
      if (t.type === "mushra" && t.responses) {
        for (let r of t.responses) {
          let row = [];
          // Agregar datos del cuestionario
          row = row.concat(participantResponses.map(x => `"${x}"`));
          // Agregar resultados de test
          row.push(`"${t.id}"`, `"${r.stimulus}"`, r.score);
          csv += row.join(",") + "\n";
        }
      }
    }

    // 💾 5. Crear archivo CSV descargable
    const blob = new Blob([csv], { type: "text/csv;charset=utf-8;" });
    const url = URL.createObjectURL(blob);

    // 🔘 6. Botón visible de descarga
    const downloadBtn = document.createElement("a");
    downloadBtn.href = url;
    downloadBtn.download = `mushra_results_${this.session.testId || "test"}_${Date.now()}.csv`;
    downloadBtn.textContent = "⬇️ Descargar resultados (.csv)";
    ...
    document.querySelector("#popupResultsContent").appendChild(downloadBtn);

    console.log("✅ Resultados generados con datos del cuestionario incluidos.");
  } catch (e) {
    console.error("❌ Error al generar CSV:", e);
    alert("Error al generar los resultados. Revisar consola.");
  }
