...
FinishPage.prototype.sendResults = function() {
  ...
  try {
    // === Generar CSV desde session.trials ===
    let csv = "trial_id,stimulus,score\n";
    const trials = this.session.trials || [];

    for (let t of trials) {
      if (t.type === "mushra" && t.responses) {
        for (let r of t.responses) {
          csv += `${t.id},${r.stimulus},${r.score}\n`;
        }
      }
    }

    // === Crear y mostrar un botón visible de descarga ===
    ...
    console.log("✅ Resultados generados. Click manual disponible para descargar CSV.");
